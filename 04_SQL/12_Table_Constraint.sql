...
-- 사용법) CREATE TABLE 테이블명(컬럼명 자료형(크기) 제약조건이름);
...
CREATE TABLE EMP_SECOND(
    ...
    ,ENAME VARCHAR2(10)
    ,SALARY NUMBER(7,2) CONSTRAINT CK_EMP_SECOND_SALARY CHECK(SALARY > 0) -- SALARY에 양수만 입력가능
);
...
-- 예제1) 사원복사본 테이블(EMP_COPY)에 기본키(PRIMARY KEY) 추가하기
...
-- 사용법 ) ALTER TABLE 테이블명 MODIFY 컬럼명 CONSTRAINT 제약조건이름 NOT NULL;

ALTER TABLE EMP_COPY
MODIFY ENAME CONSTRAINT NN_EMP_COPY_ENAME NOT NULL;

-- 제약조건이름으로 제약조건 삭제하기
-- 사용법 ) ALTER TABLE 테이블명 DROP CONSTRAINT 제약조건이름;
ALTER TABLE EMP_COPY
...
--                                  -> 있으면 INSERT/UPDATE
--                                  -> 없으면 오류발생)
--                   단점 : DB 성능저하, 최악의 경우에 다른테이블에도 성능 저하를 일으킴
--              - (2) JAVA 코딩에서 참조키 로직을 만들어서 해결
--                   if(부모테이블 해당 데이터 있는지 확인) {
--                                 INSERT 실행
--                          }
