 ...
 * 요약 :
 *          @Slf4j : 로깅 라이브러리(롬북), 인터페이스 : 자식 클래스(logback)
 *          사용법 : 예 ) 1) @Slf4j 클래스 위에 달고
 *                      2) 로깅 하고 싶은 라인에서 log.debug(값.toString())
 *          설치 : Logback 프러퍼티 파일 2개 복사, build.gradle 라이브러리 4개 추가
 *          @Service : 클래스 위에 달고, 달린 클래스는 서박 가동될 때 자동으로 객체 생성됨
 *          (IOC)
 *          예) @Repository, @Component, @Bean 등
 *          @Autowired : 위에서 생성된 객체를 가져오기 할때 사용하는 어노테이션(DI)
 *          @GetMaaping : Get 방식으로 오는 요청에(조회요청) 대해 실행되는 어노테이셙
 *          (주로 DB에 select 문이 실행됨)
 *          @PostMapping : Post 방식으로 오는 요청에(insert 요청) 대해 실행되는 어노테이션
 *          @PutMapping : Put 방식으로 오는 요청에(update 요청) 대해 실행되는 어노테이션
 *          @DeleteMapping : Delete 방식으로 오는 요청에(delete 요청) 대해 실행되는 어노테이션
