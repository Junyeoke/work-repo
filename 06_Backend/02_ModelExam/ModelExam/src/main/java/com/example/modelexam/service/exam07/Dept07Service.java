 ...
 * 요약 :
 *
 * @Service : 업무 로직을 담당하는 클래스임을 선언하는 어노테이션
 * => 스프링 서버 가동될때 그 클래스를 생성해둠(싱글톤)
 * => 객체를 만들어서 함수를 쓸때 : new 생성자함수() 하는데
 * => 스프링에서는 하지않고 미리 만들어 둔 객체를 받아서 사용함
 * (참고 :: IOC : 제어의 역전)
 * 기타 : @Repository, @Component 등
 * @Autowired : 스프링에서 미리 만들어 둔 객체를 가져오기
 * 사용법 : 변수위에 또는 생성자함수 위에 붙여 가져올 수 있음
 * (참고 :: DI : 의존성 주입)
 *
 * <p>
 * ===========================================================
...
//        TODO : deptDao.selectAll() : db 조회 함수 호출, 전체조회는 배열로 받아야함
...
//        기존 부서번호 max 찾아서 + 10
        if (dept.getDno() == null) {
            ...
            // todo : db 수정, 기본키가 없으면 insert 있으면 수정
