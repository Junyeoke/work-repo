...
//      TODO: 로그 찍기
    ...
    @GetMapping("/dept/{dno}")      // 파라메터 전달 방식
...
//        todo : 서비스 상세 조회 호출
    ...
    public RedirectView createDept(@ModelAttribute Dept dept) {
        ...
        // todo : 저장 후 전체조회 url로 페이지 강제이동
        //  사용법 : new RedirectView("강제이동될 url주소")
        return new RedirectView("/exam06/dept");
    }


    //    todo : 수정 페이지 열기 함수 : 화면이 보일 때 데이터도 화면에 미리 출력해야함
    @GetMapping("/dept/edition/{dno}")
    public String editDept(@PathVariable Long dno, Model model) {        // 기본키 필요하고, 수정된 모델도 필요
        // todo : 1) 상세 조회
        Dept dept = deptService.findById(dno);
        // todo : 화면에 찍기
        model.addAttribute("dept", dept);
    ...
    //    todo : 수정 저장 : 리다이렉트(강제이동) -> 전체 조회 페이지로 이동
    ...
    //    todo : 삭제 함수 : 삭제 후 전체 조회 페이지로 리다이렉트
