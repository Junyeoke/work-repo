...
<div class="container">
    ...
    <%--        TODO : fn 라이브러리는 el 표현식과 함께 사용 --%>
    <%--         사용법 : ${fn:contains(chars, name)} --%>
    <%--           chars에 name 값이 있으면 true 없으면 false 출력--%>
    ...
    <%--        TODO : 4. replace --%>
    <%--                   chars 문자열에서 "홍길동" -> "장길산" 변경 --%>
    <h2>replace : ${fn:replace(chars,"홍길동","장길산")}</h2>

    <%--        TODO : 5. split --%>
    <%--                   chars에서 " " 로 문자열 자르기 : 리턴값 => 배열 --%>
    ...
    <%--        TODO : 6. substring --%>
    <%--                   chars 문자열에서 0 인덱스 ~ 6-1 인덱스까지 자르기 --%>
    <h2>substring : ${fn:substring(chars,0 ,6)}</h2>

    <%--        TODO : 7. toLowerCase/toUpperCase --%>
    <%--                   대/소문자바꾸기 --%>
    ...
    <%--                TODO : 1. 숫자 찍기 : 세자리마다 쉼표 추가--%>
    <h2>formatNumber : <fmt:formatNumber value="${numtest}"/></h2>

    <%--                TODO : 2. 통화 추가--%>
    <h2>formatNumber : <fmt:formatNumber value="${numtest}" type="currency"/></h2>

    <%--                TODO : 3. 날짜 포멧 주기--%>
    <%--                 jsp 과거 코딩 : 스크립틀릿 -> el, jstl 사용권고 --%>
    <%
